...
router.get("/getVariantType", [authenticateToken.validJWTNeeded, getVariantById]);
...
async function addVariant(req, res){
    try {
        const body = req.body;
        const respone = await fndb.addNewItem(tables.variantTypes, body);
        if(respone != null){
...
async function deleteVariant(req, res){
    try {
        ...
        const response = await fndb.deleteItem(tables.variantTypes, variantId);
        if(response != null){
...
async function getVariantById(req, res){
    try {
        ...
        const variant = await fndb.getItemById(tables.variantTypes, variantId);
        if(variant != null){
...
async function editVariant(req, res) {
    try {
        const variantId = req.params.id;
        const body = req.body;
        const response = await fndb.updateItem(tables.variantTypes, variantId, body);
        if(response != null){
